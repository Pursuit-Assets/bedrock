...
// ── filters + grouping ─────────────────────────────────────────────────────────
type Field = "name" | "title" | "company" | "stage" | "has_deal" | "connected" | "last_activity" | "outreach_by";
const FILTERABLE: Record<Field, FieldMeta<JobContactWithDeal>> = {
  ...
  // Top-of-funnel triage: activity recency (date) + who did the outreach
  // (text/contains, mirroring connected-staff).
  last_activity: { label: "Last activity", type: "date", getValue: (c) => c.last_activity_at ?? "" },
  outreach_by:   { label: "Outreach by",    type: "text", getValue: (c) => (c.activity_actors ?? []).join(", ") },
};
const GROUP_OPTIONS = [
